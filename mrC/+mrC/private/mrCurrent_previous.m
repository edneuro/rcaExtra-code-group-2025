...
% 		'Stats',       'mrCG_Pivot_Options_listbox', 'mOpt', 1, { 'Mean' 'Dispersion' 'Scatter' 'SbjNames' 'Significance' }; ...
		...
		'ColorCutoff', 'mrCG_Pivot_Options_listbox', 'sOpt', 1, { '0' '1' '2' '5' '10' '20' '50' '100' 'UsrDef: ' }; ...
...
% 		gOptL = struct( 'Items', fieldnames( gOptFs ), 'Sel', 1 );
		...
		'Hems',  'mrCG_Pivot_Chart_listbox',   mrC_DefaultChartVals( 'Hems', false ), mrC_DefaultChartVals( 'Hems', true ), false; ...
		'ROIs',  'mrCG_Pivot_Chart_listbox',   [], {}, false; ...
		'ROItypes', 'mrCG_Pivot_Chart_listbox', mrC_DefaultChartVals( 'ROItypes', false ), mrC_DefaultChartVals( 'ROItypes', true ), false; ...
...
% 		gSbjROIs = [];
		...
		% start drilling...
		tSbjNms = DirCell( gProjPN, 'folders' );
...
%% -- LoadInverses
		function LoadInverses
			...
			for iSbj = 1:numel( gChartFs.Sbjs.Items )
				tSbjNm = gChartFs.Sbjs.Items{ iSbj };
				tSbjROIsPN = GetSbjROIsPN( tSbjNm );
				tBilat = gSbjROIFiles.(tSbjNm).Hem == 3;
				for iInv = 1:numel( gChartFs.Invs.Items )
					tInvNm = gChartFs.Invs.Items{ iInv };
					if isfield( gD.(tSbjNm), 'ROI' ) && isfield( gD.(tSbjNm).ROI, tInvNm )
						...
						[ gD.(tSbjNm).ROI.(tInvNm).Mean, gD.(tSbjNm).ROI.(tInvNm).SVD ] = mrC_GetROIinv( tSbjROIsPN, gSbjROIFiles.(tSbjNm).Name(tBilat), tInvM, fullfile( tInvPN, 'Inv.mat' ) );
					end
						...
						case 'Bar'
							...
							if tAvgSbjs
								...
								if IsOptSel( 'BarMean', 'Coherent' )
									tYM = abs( tY );		% abs(mean) = mean(projection onto phase of mean)
...
% 								tZ = tZ + gD.(tSbjNms{iSbj}).(tCndNms{iCnd}).(tMtg).Spec;
			...
			for iSbj = 1:tNSbjs
				...
				if (~tIsSbjPage) || (iSbj==1)
					if IsOptSel( 'TopoMap', 'elp-File' )
						tEpos = GetSensorPos( tSbjNms{iSbj}, false );
						tNE = size(tEpos,1);
	...
	% 					thetaNose = sum((tEpos(i,1)+pi*(tEpos(i,1)<0)).*tEpos(i,3))/sum(tEpos(i,3));
				...
				for iCnd = 1:tNCnds
					...
					if tIsSbjPage && (iSbj>1)
						if iSbj == tNSbjs
							set( tH(iCnd), 'FaceVertexCData', ( get( tH(iCnd), 'FaceVertexCData' ) + tZ(:) ) / tNSbjs )
						else
							set( tH(iCnd), 'FaceVertexCData', get( tH(iCnd), 'FaceVertexCData' ) + tZ(:) ) 
						end
					else
...
% 			gCortex = struct('FOV',220,'dCam',1e3,'Name','','FH',[],'AH',[],'M',[],'MH',[],'TH',[],'LH',[]);
			...
			gCortex.dCam = 1e3;
			gCortex.Name = '';
			gCortex.FH = figure( 'position', [ 400 300 400 400 ], 'tag', 'mrC_CortexFig', 'color', 'w', 'renderer', tRenderer, ...
...
% 		SetCortexFigColorMap;		% only needs update if ColorCutoff or Wave/Spec-type changed
		...
		for iSbj = 1:numel( SD.Sbjs.Items )
			...
			if ~strcmp( SD.Sbjs.Items{iSbj}, gCortex.Name )
				set( findtag( 'mrCG_Cortex_Contour_checkbox' ), 'value', 0 )
			...
			elseif get( findtag( 'mrCG_Cortex_Contour_checkbox' ), 'value' ) ~= 0
				set( findtag( 'mrCG_Cortex_Contour_checkbox' ), 'value', 0 )
				mrCG_Cortex_Contour_CB( findtag( 'mrCG_Cortex_Contour_checkbox' ) )
			end
			set( gCortex.TH, 'string', '' )
...
% 							mrCG_Cortex_Contour_CB( findtag( 'mrCG_Cortex_Contour_checkbox' ) )		*** need to udate gCortex.sensorData !!!
...
% 		tNE = size(tEpos,1);
		...
		tOrigin = fminsearch( @(tO) sphereObjFcn(tO,tEpos,128), median(tEpos) );
...
% 						tVEPInfo.nFr = tVEPInfo.nFr - 1;									% ??? should this be after error check ???
	...
	function tFilter = GetFilter( varargin )
		...
		tFilters = { ...
			'none' 'lo10' 'lo15' 'lo20' 'lo30' 'lo50' 'nf1' 'nf2' 'f2band' 'nf1clean' 'nf2clean' 'nf1low10' ...
