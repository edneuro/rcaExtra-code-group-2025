function [dataOut,W,A,Rxx,Ryy,Rxy,dGen,h] = rcaRun125(data,nReg,nComp,condRange,subjRange,show,locfile)
...
if show
    ...
    try
        for c=1:nComp
            subplot(3,nComp,c);
            topoplot(A(:,c), locs);
%             topoplot(A(:,c), locs,'electrodes','off','numcontour',0);
            title(['RC' num2str(c)]);
            axis off;
        end
    catch
