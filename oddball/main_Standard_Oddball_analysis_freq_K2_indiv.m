function main_Standard_Oddball_analysis_freq_K2_indiv

    %% define experimentInfo
    clear all; close all;
  
    code_folder = '/Users/cherrysseulsa/Desktop/research/code'; 
    ...
    experimentName = 'Indiv_test';
    
    path = '/Users/cherrysseulsa/Desktop/research/data'; % path with indiviudal folders with raw data
    cd(path);
...
%     end
    ...
    for subj = 1:length(files)
        ...
        outpath = '/Users/cherrysseulsa/Desktop/research/results/RCA'; % path for saving RCA out
...
%             analysisStruct = feval(['loadExperimentInfo_' experimentName]);
    ...
    path = '/Users/cherrysseulsa/Desktop/research/results';%for K2 study
    ...
    figure()
    for s = 1:1 %change to 2 or number of subjects
        for c = 1:6 %c is component
            subplot(s, 6, c) %rows, columns,position
            plotOnEgi(rcResult{s,1}.rcaResult.A(:,c))
            hold on
        end   
    end
   fname = '/Users/cherrysseulsa/Desktop/research/results';
  
   saveas(gca, fullfile(fname, 'RCA_test'), 'fig');
