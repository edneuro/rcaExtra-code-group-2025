function fh_AmplitudesFreqs = rcaExtra_plotAmplitudes_individual(rcaResult, plotSettings)
...
% input arguments: rcaResult structure       
    ...
    if (isempty(plotSettings))
       ...
       plotSettings = rcaExtra_getPlotSettings_individual(rcaResult);       
    ...
    for cp = 1:numel(plotSettings.RCsToPlot)
        ...
        try
            saveas(fh_AmplitudesFreqs{cp}, ...
                fullfile(rcaResult.rcaSettings.destDataDir_FIG,'condition2_WvsOLN', [fh_AmplitudesFreqs{cp}.Name '.fig']));
            close(fh_AmplitudesFreqs{cp}, ...
                fullfile(rcaResult.rcaSettings.destDataDir_FIG,'condition2_WvsOLN', [fh_AmplitudesFreqs{cp}.Name '.fig']));
        catch err
