function fh_Lolliplots = rcaExtra_plotLollipops_individual(rcaResult, plotSettings)
...
% input arguments: rcaResult structure, plotSettings (can be [])       
    ...
    for cp = 1:numel(plotSettings.RCsToPlot)
        ...
        try
            ...
            %close (fh_Lolliplots{cp}, ...
                %fullfile(rcaResult.rcaSettings.destDataDir_FIG, [fh_Lolliplots{cp}.Name '.fig']));
                saveas(fh_Lolliplots{cp}, ...
                fullfile(rcaResult.rcaSettings.destDataDir_FIG,'condition2_WvsOLN', [fh_Lolliplots{cp}.Name '.fig']));
            close (fh_Lolliplots{cp}, ...
                fullfile(rcaResult.rcaSettings.destDataDir_FIG,'condition2_WvsOLN', [fh_Lolliplots{cp}.Name '.fig']));
        catch err
