function [latencyVals, latencyErrs] = rcaExtra_computeLatenciesWithSignificance_1cond(rcaResult, statData)
...
% latencyErrs 2D array, computed latency error estimates (msec), symmetric,  nRCs x nCnd
    ...
    [~, nComp] = size(rcaResult_avg.projAvg.phase);
    latencyVals = NaN*ones(nComp);
    latencyErrs = Inf*ones(nComp);
    for rc = 1:nComp
        
        rcaAngles = squeeze(rcaResult_avg.projAvg.phase(:, rc));
        values_unwrapped = unwrapPhases(rcaAngles);
        %for c = 1:nCnd
            significance_RC = statData.sig(:, rc);            
            significantIdx = (significance_RC > 0)';
            significantFreqs = freqVals(significantIdx);
            nF = sum(significantIdx);
            if (nF >= 2)
                try
                    [Pc, ~] = polyfit(significantFreqs, values_unwrapped(significantIdx), 1);
       
                    yfit = Pc(1)*significantFreqs + Pc(2);
    
                    latencyVals(rc) = 1000*Pc(1)/(2*pi);    
                    d = (yfit - values_unwrapped(significantIdx)).^2;
                    dMs = 1000*d/(2*pi);
        
                    %dd = sqrt(sum(d)/(nF - 2)); 
                    latencyErrs(rc) = sqrt(sum(dMs)/(nF - 2));
                catch err
