...
% 		end
		...
		if ( numel( gGroups ) == 1 ) && strcmp( gGroups(1).name, 'uninitialized' ) && isempty( gGroups(1).members )
			if gProjVer == 3
				...
				for iSsn = 1:tNSbjs
					tSsnLabels{iSsn} = gD.(gChartFs.Sbjs.Items{iSsn}).SsnHeader.SsnLabel;
				end
...
% 				gVEPInfo.(tCndNm).AxxFile = gCndFiles.(tCndNm);
				...
				for iSbj = 1:numel( gChartFs.Sbjs.Items )
					...
					for iMtg = 1:numel( gChartFs.Mtgs.Items )
						...
						for iComp = 1:tNComp
% 							gD.(tSbjNm).(tCndNm).(tMtg).Harm.( gCompFieldNms{ iComp } ) = gD.(tSbjNm).(tCndNm).(tMtg).Spec( tiComp( iComp ), : );
			...
			for iSbj = 1:numel( gChartFs.Sbjs.Items )
				...
				if exist( tSsnHeaderFile, 'file' )
					gD.(tSbjNm).SsnHeader = load( tSsnHeaderFile );
					gD.(tSbjNm).SsnHeader.SsnLabel = strrep( gD.(tSbjNm).SsnHeader.SsnLabel, 'Ssn ', '' );
				elseif gProjVer == 3
					gProjVer = 2;
		...
		if IsSliceCalcItem
			...
			case 'Wave'
				if isscalar(tIndex) && ( tIndex == 0 )
					...
					tIndex = mod( tIndex-1, size( gD.(tSbjField).(tCndField).(tMtg).Wave.( tSD.Flts.Items{ tSD.Flts.Sel } ), 1 ) ) + 1;		% filtering might shorten waveform
					tEEG = gD.(tSbjField).(tCndField).(tMtg).Wave.( tSD.Flts.Items{ tSD.Flts.Sel } )( tIndex, tChans );
				end
			case {'Spec','SpecPhase'}
				if isscalar(tIndex) && ( tIndex == 0 )
					tEEG = gD.(tSbjField).(tCndField).(tMtg).Spec( :, tChans );
				else
					tEEG = gD.(tSbjField).(tCndField).(tMtg).Spec( tIndex, tChans );
				end
...
% 						tVEPInfo.nFr = tVEPInfo.nFr - 1;									% ??? should this be after error check ???
	...
	function tFilter = GetFilter( varargin )
		...
		tFilters = { ...
			...
			'nf1low15' 'nf1low20' 'nf1_odd1_odd3' 'nf1_odd3to15' 'rbtx_nf1' 'rbtx_nf2' 'rbtx_im' ...
		};
...
%% Task
	...
	function mrC_ExportData
		...
		tDomain = 'Wave';
		tFlts = fieldnames( gD.(GetChartSelx('Sbjs',1,true)).(GetChartSelx('Cnds',1,true)).(GetChartSelx('Mtgs',1,true)).Wave );
		uiH = zeros(1,11);
