...
hemi = false;	% hemisheres or whole brain flag
writeNifti = true;
...
if hemi
	classPath = fullfile(segPath,'..','left');
	if ~isdir(classPath)
		classPath = segPath;
	end
	% left
	...
	% right
	if isdir(fullfile(classPath,'..','right'))
		classPath = fullfile(classPath,'..','right');
	end
...
elseif writeNifti
    ...
    [classFile,classPath] = uiputfile('*.nii.gz','Whole brain class file',fullfile(segPath,'T1_class_ribbon.nii.gz'));
    ...
    NII.fname = fullfile(classPath,classFile);
