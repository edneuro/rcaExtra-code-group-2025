...
idx = 0;
uR = zeros(length(subjList),length(condList),2,2);
sR = zeros(length(subjList),length(condList),2,128);
vR = zeros(length(subjList),length(condList),128,128);
...
fullData(iSubj,iCond,:,:) = [real(E(kData,:)); imag(E(kData,:))];
...
figure(1)
hold on;
meanEig = diag(squeeze(mean(sR(:,1,:,:))));
errorEig  = diag(squeeze(std(sR(:,1,:,:))))/sqrt(size(sR,1));
