function [errorFlag] = jma_rois2mesh(vAnatFile,mrmFile,thresh,fWhite2Pial,roiList,colorList,autoSaveFiles,roiDir)
...
if ~exist('autoSaveFiles','var') || isempty(autoSaveFiles),
    autoSaveFiles = false;
end
...
if ~exist('roiDir','var') || isempty(roiDir)
...
for iROI = 1:nROI
    ...
    roiFilename = fullfile(roiDir,ROIfiles{iROI});
    
    if ~exist(roiFilename,'file'),
        warning(['ERROR: ' ROIfiles{iROI} ' does not exist in: ' roiDir]);
        errorFlag = true;
        continue;
    end
        
	roiCoords = load(roiFilename);		% in voxels, multiply my mmPerVox
...
% 		iMap(kGood) = iMap(kGood) + cortex.msh.nVertexLR(1);
    ...
    if exist('colorList','var') && ~isempty(colorList),
        roiCoords.ROI.color = colorList{iROI};
    end
...
% 	hist(d2(kGood),xd)
    ...
    ROIS(iROI).filename = [ROIname '.mat'];
...
if autoSaveFiles == true,
    ...
    if ~isdir(fullfile(mrmPath,'ROIs')),
        error('Cant find roioutput dir')
    end
    ROIdir = fullfile(mrmPath,'ROIs');
        
        
    if ~isnumeric(ROIdir)
        for i = 1:nROI
            ROI = ROIS(i);
            ROIfilename = fullfile(ROIdir,ROI.filename);
            
            
            disp(['writing ',ROIfilename])
            save(ROIfilename,'ROI')
        end
    else
        error(['Problem opening roidir']);
    end
end
