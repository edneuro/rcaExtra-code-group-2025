...
[uV2(iSubj,iCond,:,:) sV2(iSubj,iCond,:,:) vV2(iSubj,iCond,:,:)] = svd([real(E(k,:)); imag(E(k,:))]);


% if iSubj>1,
%     
%     for iComp = 1:2,
%         if sign(dot(squeeze(v(1,iCond,:,iComp)),squeeze(v(iSubj,iCond,:,iComp))))<0;
%             v(iSubj,iCond,:,iComp) = -v(iSubj,iCond,:,iComp);
%             u(iSubj,iCond,:,iComp) = -u(iSubj,iCond,:,iComp);
%         end
%     end
% 
% end
